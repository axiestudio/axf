...
from axf.field_typing.constants import CUSTOM_COMPONENT_SUPPORTED_TYPES, DEFAULT_IMPORT_STRING
...
def _create_axiestudio_execution_context():
    ...
    try:
        from axf.custom import Component
    ...
    try:
        from axf.io import HandleInput, Output, TabInput
...
def prepare_global_scope(module):
    ...
    for node in import_froms:
        ...
        # If original module starts with axiestudio, also try lfx equivalent
        if node.module and node.module.startswith("axiestudio."):
            lfx_module_name = node.module.replace("axiestudio.", "lfx.", 1)
...
def get_default_imports(code_string):
    ...
    axiestudio_module = importlib.import_module("lfx.field_typing")
