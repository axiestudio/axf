...
from axf.custom.utils import abuild_custom_components, create_component_template
from axf.log.logger import logger

if TYPE_CHECKING:
    from axf.services.settings.service import SettingsService
...
async def import_axiestudio_components():
    ...
    modules_dict = {}
    try:
        import axf.components as components_pkg
...
async def load_single_component(component_type: str, component_name: str, components_paths: list[str]):
    ...
    from axf.custom.utils import get_single_component_dict
...
def get_all_components(components_paths, *, as_dict=False):
    """Get all components names combining native and custom components."""
    # Import here to avoid circular imports
    from axf.custom.utils import build_custom_components
