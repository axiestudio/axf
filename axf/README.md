# axf - AxieStudio Executor

axf is a command-line tool for running AxieStudio workflows. It provides two main commands: `serve` and `run`.
...
uvx axf serve my_flow.json
uvx axf run my_flow.json "input"
```
...
cd axiestudio/src/axf
uv run axf serve my_flow.json
```
...
axf now supports simplified component imports for better developer experience:

**Before (old import style):**

```python
from axf.components.agents.agent import AgentComponent
from axf.components.data.url import URLComponent
from axf.components.input_output import ChatInput, ChatOutput
```

**Now (new flattened style):**

```python
from axf import components as cp
...
uv run axf serve my_flow.json --port 8000
```
...
# Start server
uv run axf serve simple_chat.json --host 0.0.0.0 --port 8000
...
### `axf run` - Run flows directly

Execute an AxieStudio workflow and get results immediately.

```bash
uv run axf run my_flow.json "What is AI?"
```
