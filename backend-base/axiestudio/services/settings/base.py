...
# in the axiestudio directory
    ...
    """Directory where knowledge bases are stored. If not set, defaults to a 'knowledge_bases' directory in the config directory."""
    ...
    @classmethod
    def set_knowledge_bases_dir(cls, value):
        """Set default knowledge bases directory if not provided."""
        if not value:
            # Default to a 'knowledge_bases' directory in the config directory
            config_dir = os.getenv("AXIESTUDIO_CONFIG_DIR", ".")
            value = os.path.join(config_dir, "knowledge_bases")
            logger.debug(f"Setting default knowledge bases directory to {value}")
        return value
