...
async def cancel_flow_build(
    *,
    ...
    # FIXED: Cancel the task but DON'T call cleanup_job immediately
    # Let the periodic cleanup handle it after grace period
    if not event_task.done():
        logger.info(f"Cancelling task for job_id {job_id}")
        event_task.cancel()

        # Wait briefly for cancellation to take effect
        try:
            await asyncio.wait([event_task], timeout=1.0)
        except asyncio.TimeoutError:
        ...
        if event_task.cancelled():
            ...
            logger.error(f"Failed to cancel task for job_id {job_id}")
            return False
    else:
        logger.info(f"Task for job_id {job_id} is already completed")
        return True
