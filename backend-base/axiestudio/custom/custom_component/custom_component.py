...
class CustomComponent(BaseComponent):
    ...
    def resolve_path(path: str) -> str:
        ...
        # Handle Docker/container paths that need to be mapped to local storage
        if path.startswith("/app/data/.cache/axiestudio/"):
            # Extract flow_id and file_name from Docker path
            # Format: /app/data/.cache/axiestudio/{flow_id}/{file_name}
            relative_path = path.replace("/app/data/.cache/axiestudio/", "")
            if "/" in relative_path:
                flow_id, file_name = relative_path.split("/", 1)
                from axiestudio.services.deps import get_storage_service
                storage_svc = get_storage_service()
                return storage_svc.build_full_path(flow_id, file_name)

        # Handle other Docker paths by mapping to local cache
        if path.startswith("/app/data/"):
            from axiestudio.services.cache.utils import CACHE_DIR
            relative_path = path.replace("/app/data/", "")
            return str(Path(CACHE_DIR) / relative_path)
